...
import { annualJobRecordSchema, annualResultsDatasetSchema, annualSimulationRequestSchema, localProjectSnapshotSchema, } from "@agrivoltaic/shared";
import { assertSafeJobId, ensureDirectory, normalizeDirectoryPath, pathExists, readJson, resolveJobDirectory, writeJson, } from "../annual/storage.js";
export const PROJECT_FILE_NAME = "agrivoltaic-project.json";
const PROJECT_VERSION = "1";
function normalizePath(path, label = "Project folder path") {
    return normalizeDirectoryPath(path, label);
}
function defaultProjectName(rootPath, explicitName) {
...
function resolveProjectRoot(basePath, projectName) {
...
function createSnapshot(rootPath, projectName) {
...
async function resolveExistingPath(rootPath, candidates, label) {
...
function importedJobId(explicitJobId) {
...
function buildImportedJobSummary(jobId, request, dataset) {
...
export class ProjectService {
    async openProjectFolder(input) {
        ...
        const stored = localProjectSnapshotSchema.parse(await readJson(projectFilePath));
    ...
    async saveProjectSnapshot(input) {
        ...
        const existing = await pathExists(projectFilePath)
            ? localProjectSnapshotSchema.parse(await readJson(projectFilePath))
    ...
    async importHpcResults(input) {
        ...
        const request = annualSimulationRequestSchema.parse(await readJson(requestPath));
