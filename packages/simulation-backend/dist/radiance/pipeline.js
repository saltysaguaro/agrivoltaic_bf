...
import { BACKEND_VERSION, DEFAULT_SIMULATION_OPTIONS, exportPackageRequestSchema, } from "@agrivoltaic/shared";
...
function commandShellLine(command) {
...
function writeablePackageFile(relativePath, contents, contentType) {
    return {
        relativePath,
        contents,
...
async function writeRelativeFile(packageRoot, file) {
    const target = join(packageRoot, file.relativePath);
    await mkdir(dirname(target), { recursive: true });
    await writeFile(target, file.contents, "utf8");
    return target;
}
function buildRtraceCommand(gridId, options) {
    const args = [
        "-I+",
...
export async function createExportPackage(requestInput) {
    ...
    geometryFiles.push(writeablePackageFile("geometry/scene-manifest.json", JSON.stringify(request.sceneExport.sceneManifest, null, 2), "application/json"));
