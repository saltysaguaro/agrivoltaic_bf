...
export function solarPosition(timestamp, latitude, longitude) {
    ...
    const azimuth = radToDeg(Math.atan2(Math.sin(hourAngle), (Math.cos(hourAngle) * Math.sin(latRad)) - (Math.tan(decl) * Math.cos(latRad)))) + 180;
    return {
        elevationDeg: elevation,
        azimuthDeg: azimuth,
    };
}
function monthIndexFromTimestamp(timestamp) {
    return timestamp.getUTCMonth() + 1;
}
function buildWeatherSummary(site, source, hourly, extras = {}) {
...
export function buildSyntheticWeather(site, year = 2025) {
    const hourly = [];
    let hourIndex = 0;
...
export async function fetchPvgisTmy(site) {
    ...
    const hourly = rows.map((row, hourIndex) => {
...
function lineHasHourlyColumns(values) {
...
export function parseNsrdbTmyCsv(csv, site) {
...
export async function fetchNsrdbTmy(site) {
    ...
    for (const datasetName of datasetCandidates) {
        ...
        const response = await fetch(`https://developer.nrel.gov/api/nsrdb/v2/solar/nsrdb-GOES-tmy-v4-0-0-download.csv?${params.toString()}`);
        if (!response.ok) {
...
export async function acquireAnnualWeather(site) {
    if (process.env.AGRIVOLTAIC_WEATHER_MODE === "synthetic") {
        if (process.env.AGRIVOLTAIC_ALLOW_SYNTHETIC !== "true") {
            throw new Error("[WEATHER_SYNTHETIC_DISABLED] Synthetic weather is disabled for the real app. "
