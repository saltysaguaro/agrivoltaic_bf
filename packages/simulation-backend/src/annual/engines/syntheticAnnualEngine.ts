import type {
  ...
  ExportPackageResult,
  GridMonthlyAggregate,
  SensorMonthlyAggregate,
  SensorPoint,
...
function systemTypeFromRequest(request: AnnualSimulationRequest): string {
  ...
  const serializedType = serializedSystem && typeof serializedSystem === "object"
    ? (serializedSystem as Record<string, unknown>)["type"]
    : undefined;
...
function monthlyExposureForSensor(
  sensor: SensorPoint,
  monthIndex: number,
  systemType: string,
...
function buildSensorAggregate(
  ...
  return {
    ...
    monthlyIrradianceWhM2: Array.from({ length: 12 }, (_, monthIndex) => (
      monthlyExposureForSensor(sensor, monthIndex, systemType, weather, classifications)
    )),
  };
}
...
export async function runSyntheticAnnualEngine(input: {
  ...
  const monthlyAggregates: GridMonthlyAggregate[] = input.exportPackage.grids.map((grid) => ({
    gridId: grid.gridId,
    classification: grid.classifications,
    dimensions: grid.dimensions,
    rowPairId: grid.rowPairId,
    bayId: grid.bayId,
    sensors: grid.sensors.map((sensor) => buildSensorAggregate(
      sensor,
      systemType,
      input.weather,
