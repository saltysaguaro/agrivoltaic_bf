...
import {
  detectBifacialRadiance,
  resolveAnnualPythonCommand,
  runBifacialRadianceEngine,
} from "./engines/bifacialRadianceEngine.js";
import { runSyntheticAnnualEngine } from "./engines/syntheticAnnualEngine.js";

function defaultSkyForSite(request: AnnualSimulationRequest) {
  return {
...
function summarizeJob(record: AnnualJobRecord): AnnualJobSummary {
  return {
    jobId: record.jobId,
...
function appendNotes(record: AnnualJobRecord, ...notes: string[]): string[] {
  return [...record.notes, ...notes];
}

function buildPortableJobSummary(request: AnnualSimulationRequest, weatherSource: AnnualJobSummary["weatherSource"]): AnnualJobSummary {
...
async function selectEngine(preference?: AnnualSimulationRequest["enginePreference"]): Promise<AnnualJobRecord["engine"]> {
  const envPreference = process.env.AGRIVOLTAIC_ENGINE as AnnualJobRecord["engine"] | undefined;
  ...
  if (desired === "synthetic_local") {
    if (process.env.AGRIVOLTAIC_ALLOW_SYNTHETIC !== "true") {
      throw new Error(
        "Synthetic irradiance modeling is disabled for the real app. "
  ...
  if (!available) {
    throw new Error(
      `bifacial_radiance is required for irradiance modeling, but it is not importable from ${pythonCommand}. `
...
function sensorPointsText(grid: SensorGridVolume): string {
  return `${grid.sensors.map((sensor) => (
    `${sensor.position.x.toFixed(6)} ${sensor.position.y.toFixed(6)} ${(sensor.position.z + 0.05).toFixed(6)} `
    + `${sensor.normal.x.toFixed(6)} ${sensor.normal.y.toFixed(6)} ${sensor.normal.z.toFixed(6)}`
  )).join("\n")}\n`;
}

function filterGridsBySelection(grids: SensorGridVolume[], selectedSensorIds?: string[]): SensorGridVolume[] {
  ...
  const filtered = grids
    ...
    .filter(Boolean) as SensorGridVolume[];
...
function applySensorSelectionToExportPackage(
...
function runAnywhereReadme(request: AnnualSimulationRequest, weatherSource: string, sensorCount: number): string {
...
function periodDayCount(dataset: AnnualResultsDataset, includedMonths: readonly number[]): number {
...
export class AnnualJobService {
  private readonly jobs = new Map<string, AnnualJobRecord>();
  ...
  private buildJobRecord(request: AnnualSimulationRequest, engine: AnnualJobRecord["engine"]): AnnualJobRecord {
    const createdAt = nowIso();
    const qualityPreset = resolveAnnualSimulationQualityPreset(request.simulationQualityPreset);
  ...
  private resolveJobDir(jobId: string, projectRoot?: string): string {
    return resolveJobDirectory(jobId, projectRoot || this.dataRoot);
  }

  private async persistJob(record: AnnualJobRecord): Promise<void> {
  ...
  private setJob(record: AnnualJobRecord): AnnualJobRecord {
    this.jobs.set(record.jobId, record);
    return record;
  }

  private async updateJob(jobId: string, patch: Partial<AnnualJobRecord>): Promise<AnnualJobRecord> {
  ...
  async createJob(requestInput: unknown): Promise<AnnualJobSummary> {
  ...
  async exportRunAnywherePackage(requestInput: unknown): Promise<{
    packageRoot: string;
    ...
    await Promise.all(exportPackage.grids.map((grid) => writeText(
      join(exportPackage.packageRoot, "sensors", `${grid.gridId}.pts`),
      grid.radiancePoints,
    )));
    ...
    const runnerSource = await readFile(
      fileURLToPath(new URL("../../python/run_bifacial_annual.py", import.meta.url)),
      "utf8",
    );
...
`;
    ...
    await writeText(
      join(exportPackage.packageRoot, "README.md"),
      runAnywhereReadme(request, weather.providerLabel ?? weather.source, selectedSensorCount),
    );
  ...
  private async runJob(jobId: string, request: AnnualSimulationRequest): Promise<void> {
    const qualityPreset = resolveAnnualSimulationQualityPreset(request.simulationQualityPreset);
    ...
    record = await this.updateJob(jobId, {
      ...
      notes: appendNotes(
        record,
        `Geometry packaged with ${exportPackage.grids.reduce((sum, grid) => sum + grid.sensors.length, 0)} selected sensors. Acquiring 8760 weather data.`,
        `Radiance tracing quality preset resolved to ${ANNUAL_SIMULATION_QUALITY_PRESETS[qualityPreset].label}.`,
      ),
    });
    ...
    record = await this.updateJob(jobId, {
      ...
      notes: appendNotes(
        record,
        `Weather source: ${weather.providerLabel ?? weather.source}.`,
        ...(weather.notes ?? []),
      ),
    });
    ...
    let dataset: AnnualResultsDataset;
  ...
  async getJobRecord(jobId: string, projectRoot?: string): Promise<AnnualJobRecord> {
    ...
    for (const root of candidateRoots) {
      ...
      const record = annualJobRecordSchema.parse(await readJson<AnnualJobRecord>(recordPath));
  ...
  async getJob(jobId: string, projectRoot?: string): Promise<AnnualJobSummary> {
    return summarizeJob(await this.getJobRecord(jobId, projectRoot));
  }

  async getJobLogs(jobId: string, projectRoot?: string): Promise<{
    tail: string;
    lineCount: number;
    ...
    return {
      ...
      progress: await pathExists(progressPath)
        ? await readJson<Record<string, unknown>>(progressPath)
        : null,
    };
  }

  async getDataset(jobId: string, projectRoot?: string): Promise<AnnualResultsDataset> {
    const record = await this.getJobRecord(jobId, projectRoot);
    if (!record.resultPath) {
      throw new Error(`Job ${jobId} does not have completed results yet`);
    }
    return annualResultsDatasetSchema.parse(await readJson<AnnualResultsDataset>(record.resultPath)) as AnnualResultsDataset;
  }

  async getMetadata(jobId: string, projectRoot?: string): Promise<AnnualResultsMetadata> {
    const record = await this.getJobRecord(jobId, projectRoot);
    const dataset = await this.getDataset(jobId, projectRoot);
    const heightLevelsM = [...new Set(
    ...
    return annualResultsMetadataSchema.parse({
      ...
      designState: annualSimulationRequestSchema.parse(
        await readJson(join(this.resolveJobDir(jobId, record.projectRoot ?? projectRoot), "request.json")),
      ).designState,
  ...
  async getResultsView(jobId: string, requestInput: unknown, projectRoot?: string): Promise<AnnualResultsView> {
  ...
  async exportJobBundle(jobId: string, projectRoot?: string): Promise<unknown> {
    const dataset = await this.getDataset(jobId, projectRoot);
