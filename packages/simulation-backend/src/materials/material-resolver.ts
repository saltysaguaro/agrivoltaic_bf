...
export function validateManifestMaterials(
...
export function toRadianceMaterial(material: MaterialDefinition): string {
  ...
  switch (material.modifier) {
    ...
    default:
      throw new Error(`Unsupported Radiance modifier ${String((material as { modifier: unknown }).modifier)}`);
  }
}

export function buildMaterialFile(materials: MaterialDefinition[]): string {
