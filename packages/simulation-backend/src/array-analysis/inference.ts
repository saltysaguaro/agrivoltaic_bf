...
interface InferredArrayContext {
...
function simulationBoundsFor(record: SceneObjectRecord): Bounds3 {
  return record.simulationBounds ?? record.bounds;
}

function defaultArrayId(record: SceneObjectRecord): string {
  return record.metadata.arrayId ?? "array-1";
}

function dominantHorizontalAxis(points: Vec3[]): Vec3 {
  if (points.length <= 1) {
    return { x: 1, y: 0, z: 0 };
  }
...
function dominantHorizontalAxisFromBounds(bounds: Bounds3): Vec3 {
...
function averageAlignedAxes(axes: Vec3[], fallback: Vec3): Vec3 {
  if (axes.length === 0) {
    return fallback;
  }
...
function groupedHorizontalAxis(
  records: SceneObjectRecord[],
  centroids: Vec3[],
  keySelector: (record: SceneObjectRecord) => string | undefined,
  fallback: Vec3,
): Vec3 | null {
...
function inferArrayRowAxis(records: SceneObjectRecord[], boundsList: Bounds3[], centroids: Vec3[]): Vec3 {
  const centroidAxis = dominantHorizontalAxis(centroids);
  const explicitRowAxis = groupedHorizontalAxis(
    records,
    centroids,
    (record) => record.metadata.rowId,
    centroidAxis,
  );
  ...
  const explicitBayCrossAxis = groupedHorizontalAxis(
    records,
    centroids,
    (record) => record.metadata.bayId,
    { x: -centroidAxis.y, y: centroidAxis.x, z: 0 },
  );
...
function projectBounds(bounds: Bounds3, origin: Vec3, eRow: Vec3, eCross: Vec3): Omit<ProjectedModule, "record" | "bounds" | "centroid" | "bayId"> {
...
function groupByRow(
...
): Array<{ rowId: string; modules: ProjectedModule[] }> {
  const allHaveRowIds = modules.every((module) => Boolean(module.record.metadata.rowId));
  if (allHaveRowIds) {
    const grouped = new Map<string, ProjectedModule[]>();
  ...
  const rows: Array<{ rowId: string; modules: ProjectedModule[] }> = [];

  let current: ProjectedModule[] = [];
  let centerCross = 0;
...
function representativeModuleSpan(modules: ProjectedModule[]): number {
  const spans = modules
    ...
    for (const cross of crossVectors) {
      for (const z of zValues) {
        corners.push(addVec3(
          addVec3(
            addVec3(input.origin, scaleVec3(input.eRow, along)),
            scaleVec3(input.eCross, cross),
          ),
          { x: 0, y: 0, z },
        ));
      }
...
function buildDerivedBaySpans(
  overlapAlongRow: [number, number],
  modules: ProjectedModule[],
  config: SensorGridConfig,
): Array<{ spanAlongRow: [number, number] }> {
  ...
  return Array.from({ length: bayCount }, (_, index) => {
    ...
    return {
      spanAlongRow: [start, end] as [number, number],
    };
  });
}

function buildBayDescriptors(
  array: ArrayDescriptor,
  rows: RowDescriptor[],
  rowModules: Map<string, ProjectedModule[]>,
  config: SensorGridConfig,
): BayDescriptor[] {
  if (rows.length < 2) {
    return [];
  ...
  rows.slice(0, -1).forEach((row, index) => {
    ...
    const overlapAlongRow: [number, number] = [
    ...
    baySpans.forEach((span, bayIndex) => {
      ...
      const center = addVec3(
        addVec3(array.localFrame.origin, scaleVec3(array.localFrame.eRow, centerAlong)),
...
function inferArrayContext(
  arrayId: string,
  records: SceneObjectRecord[],
  config: SensorGridConfig,
): InferredArrayContext {
  ...
  const rows = groupedRows
    .map(({ rowId, modules }) => {
      ...
      const rowAxis = alignAxis(
        modules.length > 1
          ? dominantHorizontalAxis(rowCenters)
          : dominantHorizontalAxisFromBounds(rowBounds),
        eRow,
      );
      ...
      return {
        ...
        rowIndex: 0,
      } satisfies RowDescriptor;
    })
    .sort((a, b) => a.centerCross - b.centerCross)
    .map((row, rowIndex) => ({ ...row, rowIndex }));

  const array: ArrayDescriptor = {
    arrayId,
    bounds: arrayBounds,
    ...
    localFrame: {
      ...
      eCross,
      eUp,
    } satisfies LocalFrame,
    rowIds: rows.map((row) => row.rowId),
    rowCount: rows.length,
  };

  const rowPairs: RowPairDescriptor[] = rows.slice(0, -1).map((row, index) => {
    const nextRow = rows[index + 1]!;
    const overlapAlongRow: [number, number] = [
      Math.max(row.alongMin, nextRow.alongMin),
      Math.min(row.alongMax, nextRow.alongMax),
...
export function inferFullArrayAnalysis(
  ...
  const groupedByArray = new Map<string, SceneObjectRecord[]>();
  for (const module of modules) {
  ...
  const rowPairs: RowPairDescriptor[] = [];
  const bays: BayDescriptor[] = [];
