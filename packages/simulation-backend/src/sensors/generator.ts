...
function axisOffsets(length: number, count: number, placement: AxisPlacement = "boundary"): number[] {
  if (count <= 1) {
    return [0];
  }
...
function combine(frame: LocalFrame, rowOffset: number, crossOffset: number, upOffset: number): Vec3 {
  return addVec3(
    addVec3(
      addVec3(frame.origin, scaleVec3(frame.eRow, rowOffset)),
      scaleVec3(frame.eCross, crossOffset),
    ),
    scaleVec3(frame.eUp, upOffset),
  );
}

export function sensorVolumeWorldBounds(frame: LocalFrame, bounds: SensorGridBounds) {
...
export function buildSensorGridVolume(input: BuildSensorGridVolumeInput): SensorGridVolume {
  ...
  for (let i = 0; i < nx; i++) {
    for (let j = 0; j < ny; j++) {
      for (let k = 0; k < nz; k++) {
        ...
        sensors.push(sensor);
        const radiancePosition = addVec3(
          position,
          scaleVec3(input.frame.eUp, RADIANCE_TRACE_OFFSET_M),
        );
        lines.push([
