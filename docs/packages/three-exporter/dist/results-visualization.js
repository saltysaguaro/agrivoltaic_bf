import { BufferGeometry, Color, Float32BufferAttribute, Group, InstancedMesh, Matrix4, MeshBasicMaterial, Object3D, PlaneGeometry, Points, PointsMaterial, SphereGeometry, Vector3, } from "three";
function inferValueRange(results) {
...
function colorForValue(value, range) {
    ...
    return new Color().setRGB(Math.max(0, Math.min(1, (t - 0.15) * 1.9)), Math.max(0, Math.min(1, 1.1 - Math.abs((t - 0.5) * 2.2))), Math.max(0, Math.min(1, (0.78 - t) * 1.55)));
}
function matchesFilter(grid, filter) {
    if (!filter)
        return true;
    if (filter.gridIds && !filter.gridIds.includes(grid.gridId))
        return false;
...
function collectSamples(results, filter) {
    return results.grids
        .filter((grid) => matchesFilter(grid, filter))
        .flatMap((grid) => grid.irradianceResults);
}
export function createSensorPointCloud(results, options = {}) {
...
export function createPerGridPointGroups(results, options = {}) {
    const range = options.range ?? inferValueRange(results);
...
export function createInstancedSensorMarkers(results, options = {}) {
    const samples = collectSamples(results, options.filter);
    const range = options.range ?? inferValueRange(results);
    const mesh = new InstancedMesh(new SphereGeometry(options.size ?? 0.08, 10, 10), new MeshBasicMaterial({ vertexColors: true }), samples.length);
...
function getGridOrThrow(results, gridId) {
    const grid = gridId
...
export function createSliceHeatmap(results, options) {
    ...
    for (const sample of grid.irradianceResults) {
        if (sample.indices[axisIndex] !== options.index)
            continue;
        const color = colorForValue(sample.Ee, range);
        let geometry;
        ...
        const mesh = new InstancedMesh(geometry, new MeshBasicMaterial({
            color,
            transparent: true,
...
export function createVolumeLayerView(results, axis, layerEvery = 4, filter) {
    const root = new Group();
    root.name = `volume-layers-${axis}`;
    for (const grid of results.grids) {
        if (!matchesFilter(grid, filter))
            continue;
...
export function buildLegendStops(results, steps = 6) {
...
export function summarizeEdgeVsInterior(results) {
    const lines = [
