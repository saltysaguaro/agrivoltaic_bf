...
function buildRadianceChildEnv(pythonCommand) {
...
export function resolveAnnualPythonCommand() {
    return process.env.AGRIVOLTAIC_PYTHON?.trim() || DEFAULT_ANNUAL_PYTHON;
}
export async function detectBifacialRadiance(pythonCommand = resolveAnnualPythonCommand()) {
...
export async function runBifacialRadianceEngine(input) {
    ...
    await new Promise((resolve, reject) => {
        ...
        const child = spawn(pythonCommand, [runnerPath, "--input", inputPath, "--output", outputPath, "--log", runnerLogPath, "--progress", progressPath], {
    ...
    const dataset = await readJson(outputPath);
    return annualResultsDatasetSchema.parse(dataset);
}
export function parseBifacialDataset(input) {
    return annualResultsDatasetSchema.parse(input);
}
