...
import { exportPackageRequestSchema, importResultsRequestSchema, } from "@agrivoltaic/shared";
...
export async function createSimulationPackage(requestInput) {
    return createExportPackage(requestInput);
}
export async function importSimulationResults(requestInput) {
    const request = importResultsRequestSchema.parse(requestInput);
    return importRadianceResults(request);
}
export async function runSimulation(requestInput) {
    const request = exportPackageRequestSchema.parse(requestInput);
    ...
    await writeFile(join(packageResult.packageRoot, "logs", "command-log.json"), JSON.stringify(commandLog, null, 2), "utf8");
    ...
    await writeFile(join(packageResult.packageRoot, "results", "imported-results.json"), JSON.stringify(importedResults, null, 2), "utf8");
    return {
        simulationId,
        exportPackageId: packageResult.exportPackageId,
