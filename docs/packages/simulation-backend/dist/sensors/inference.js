import { DEFAULT_SENSOR_CONFIG, DEFAULT_SENSOR_MARGINS, addVec3, scaleVec3, } from "@agrivoltaic/shared";
import { inferFullArrayAnalysis } from "../array-analysis/inference.js";
import { buildSensorGridVolume } from "./generator.js";
function rowPairMatches(rowIds, selectedRowIds) {
    if (!selectedRowIds)
        return true;
    return ((rowIds[0] === selectedRowIds[0] && rowIds[1] === selectedRowIds[1])
...
function chooseRepresentativeRowPair(analysis, rowPairs) {
...
function manualGrid(configInput) {
...
function getRowPairContext(analysis, rowPair) {
...
function centeredSpan(overlap, desiredLength) {
...
function rowCountForLength(length, targetSpacing, minimumCount) {
    if (length <= 0) {
...
function paddingCellsForLength(length, cellSize) {
    if (length <= 1e-6 || cellSize <= 1e-6) {
        return 0;
    }
    return Math.max(0, Math.ceil(length / cellSize));
}
function createGridFromRowPair(analysis, rowPair, config, index, totalCount) {
    ...
    const frameOrigin = addVec3(addVec3(addVec3(planarOrigin, scaleVec3(array.localFrame.eRow, centerAlong)), scaleVec3(array.localFrame.eCross, centerCross)), scaleVec3(array.localFrame.eUp, groundElevation + margins.bottomOffset + (height * 0.5)));
    return buildSensorGridVolume({
        ...
        dimensions: [rowCount, crossCount, heightCount],
        bayIndex: index,
        bayCount: totalCount,
    });
}
function createUniformArrayGridFromArray(analysis, array, config, index, totalCount) {
...
export function inferSensorGridVolumes(manifest, configInput = {}) {
    const config = {
    ...
    if (config.mode === "fullArrayGrid" && config.fullArrayTilingStrategy === "uniformArrayGrid") {
        ...
        grids = candidateArrays.map((array, index) => createUniformArrayGridFromArray(analysis, array, config, index, candidateArrays.length));
        representativeGridId = grids[0]?.gridId;
    }
    else {
        grids = candidateRowPairs.map((rowPair, index) => createGridFromRowPair(analysis, rowPair, config, index, candidateRowPairs.length));
