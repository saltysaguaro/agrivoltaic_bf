import { importResultsRequestSchema, } from "@agrivoltaic/shared";
import { buildGridResult, summarizeArray, summarizeClassifications, } from "./stats.js";
...
function rgbToIrradiance(parts) {
    const [r, g, b] = parts;
    return (0.265 * r) + (0.67 * g) + (0.065 * b);
}
function parseJsonValues(content) {
...
function parseResultValues(file) {
...
function filenameMatchGrid(fileName, grids) {
    const lower = fileName.toLowerCase();
    return grids.find((grid) => lower.includes(grid.gridId.toLowerCase()));
}
function appendValues(target, gridId, values) {
    const existing = target.get(gridId) ?? [];
    existing.push(...values);
    target.set(gridId, existing);
}
function assignValuesToGrids(grids, files, notes) {
    const valuesByGrid = new Map();
    const unmatched = [];
...
function samplesForGrid(grid, values) {
...
export function importRadianceResults(requestInput) {
