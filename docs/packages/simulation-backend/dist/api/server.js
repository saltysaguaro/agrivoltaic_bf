...
import { createSimulationPackage, importSimulationResults, runSimulation, } from "../runner/simulation-runner.js";
import { InMemoryJobStore } from "../runner/job-store.js";
import { AnnualJobService } from "../annual/job-service.js";
import { MAPBOX_NOT_CONFIGURED_MESSAGE, MapboxNotConfiguredError, MapboxResolutionError, resolveSitePlace, searchSitePlaces, } from "../site/mapbox.js";
import { ProjectService } from "../projects/project-service.js";
import { normalizeDirectoryPath } from "../annual/storage.js";
export const STATIC_PAGES = new Map([
    ["/", "index.html"],
...
export function createSimulationServer() {
    ...
    app.get("/api/config/client", async (_req, res) => {
        res.status(200).json(await annualJobs.getClientConfig());
    });
    app.get("/api/site/autocomplete", async (req, res) => {
    ...
    app.get("/api/site/resolve", async (req, res) => {
        const query = String(req.query.q ?? "").trim();
        const token = process.env.MAPBOX_ACCESS_TOKEN?.trim() ?? "";
        if (!token) {
            res.status(503).json({ error: MAPBOX_NOT_CONFIGURED_MESSAGE });
    ...
    app.post("/api/jobs/annual", async (req, res) => {
        try {
    ...
    app.post("/api/jobs/export-run-anywhere", async (req, res) => {
        try {
    ...
    app.post("/api/projects/pick-folder", async (_req, res) => {
        try {
    ...
    app.post("/api/projects/pick-hpc-folder", async (_req, res) => {
        try {
    ...
    app.post("/api/projects/import-hpc-results", async (req, res) => {
        try {
    ...
    app.get("/api/jobs/:id/results/metadata", async (req, res) => {
        try {
            const jobId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    ...
    app.get("/api/jobs/:id/results/view", async (req, res) => {
        try {
            const jobId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    ...
    app.post("/export-package", async (req, res) => {
        try {
    ...
    app.post("/import-results", async (req, res) => {
        try {
    ...
    app.get("/simulation/:id/status", (req, res) => {
        const jobId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    ...
    app.get("/simulation/:id/result", (req, res) => {
        const jobId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
        const record = jobs.get(jobId);
        if (!record) {
            res.status(404).json({ error: "Simulation not found" });
    ...
    app.use(PUBLIC_STATIC_DIRS[1], express.static(join(rootDir, "packages", "shared", "dist"), { index: false }));
    app.use(PUBLIC_STATIC_DIRS[2], express.static(join(rootDir, "packages", "three-exporter", "dist"), { index: false }));
    app.use(PUBLIC_STATIC_DIRS[3], express.static(join(rootDir, "packages", "simulation-backend", "dist"), { index: false }));
    app.use(PUBLIC_STATIC_DIRS[4], express.static(join(rootDir, "node_modules", "three"), { index: false }));
    app.use(PUBLIC_STATIC_DIRS[5], express.static(join(rootDir, "node_modules", "zod"), { index: false }));
    ...
    app.get("/public-visualizer", (_req, res) => {
        res.redirect(302, "/public-visualizer.html");
    });
    app.get("/model/run/:id", (_req, res) => {
