...
function middleBandSensorIds(grids, bandWidth = 10) {
  const sensors = grids.flatMap((grid) => grid.sensors);
...
async function bootSensorLayoutPage() {
  ...
  const selectedSensorIds = new Set(savedSelection?.selectedSensorIds || []);
  ...
  function refreshSummary() {
    ...
    selectionInfo.innerHTML = `
      ...
      <div><span>Sensor lattice</span><strong>${activeAcrossCount} between rows × dynamic along-row × ${activeHeightCount} heights</strong></div>
  ...
  function applyPreset(preset) {
    ...
    if (preset === "allCurrent") {
      setSelected(sensorsAtCurrentHeight().map((sensor) => sensor.id), "merge");
      return;
    }
    ...
    if (preset === "withinArrayCurrent") {
      setSelected(sensorsAtCurrentHeight().filter(sensorWithinArray).map((sensor) => sensor.id), "merge");
      return;
    }

    if (preset === "middleRow") {
      setSelected(middleBandSensorIds(grids), "merge");
      return;
    }
