...
import { centeredSquareSensorIds } from "../platform/sensorSelectionPresets.js";
import {
  createSelectableSensorOverlay,
  updateSelectableOverlayColors,
} from "../platform/sensorSelectionOverlay.js";
import { bindSceneViewportControls } from "../platform/sceneViewportControls.js";
import {
  buildAnnualSimulationPayload,
  buildSelectableSensorConfig,
} from "../platform/sceneExport.js";
import {
  buildPublicStudyBundle,
  downloadPublicStudyBundle,
} from "./publicStudyBundle.js";
import {
...
async function bootPublicStudySensorPage() {
  ...
  if (!designState) {
    window.location.replace(PUBLIC_STUDY_DESIGN_HREF);
    return;
  }
  ...
  const exportButton = document.getElementById("downloadStudyBundleButton");
  ...
  bindSceneViewportControls({
    ...
    exportFileName: "agrivoltaic-study-sensor-layout.png",
  ...
  const schedulePersistSelection = debounce(persistSelection, 250);
  ...
  function refreshSummary() {
    ...
    selectionInfo.innerHTML = `
      <div><span>Study</span><strong>${projectName}</strong></div>
      ...
      <div><span>Export</span><strong>Work package for local or HPC execution</strong></div>
    `;
  }
  ...
  function refreshOverlay() {
    ...
    refreshSummary();
    status.textContent = `${visibleCells().length.toLocaleString()} sensors shown at ${buildHeightLabel(heightLevelsM, currentHeightIndex)}.`;
  }
  ...
  exportButton.addEventListener("click", () => {
    if (!selectedSensorIds.size) {
      status.textContent = "Select at least one sensor before exporting the work package.";
      return;
    }

    exportButton.disabled = true;
    status.textContent = "Building work package...";
    try {
      ...
      const request = buildPayload(sensorSelection.selectedSensorIds);
      const bundle = buildPublicStudyBundle({
        request,
        sensorSelection,
      });
      session.setSensorSelection(sensorSelection);
      downloadPublicStudyBundle(bundle);
      status.textContent = "Work package downloaded.";
    } catch (error) {
      status.textContent = error instanceof Error ? error.message : "Could not build the work package.";
    } finally {
...
bootPublicStudySensorPage().catch((error) => {
  const status = document.getElementById("sensorLayoutStatus");
  if (status) {
    status.textContent = error instanceof Error ? error.message : "Could not load the sensor workspace.";
  }
});
